...
# #2. 모델구성
model = Sequential()
model.add(Dense(10, input_shape=(28, 28)))     # (None , 28, 100)  # Dense 레이어는 2D 데이터를 받기 때문에 1D로 변환 필요
model.add(Reshape(target_shape=(28, 10 ,1)))   # (None, 28, 10 , 1)    # Reshape 레이어를 사용하여 2D 데이터를 3D로 변환
model.add(Conv2D(20, (3,3), strides=1,))  # (None, 26, 8, 20)
model.add(Conv2D(filters=20, kernel_size=(3,3)))                   # 24,6,20
model.add(Flatten())
model.add(Dropout(0.2))     # Conv layer도 Dropout 사용 가능.
model.add(BatchNormalization())
model.add(Reshape(target_shape=(24, 120)))
model.add(LSTM(20))
