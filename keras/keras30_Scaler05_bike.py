...
def apply_and_train_with_scalers(X, y, test_size=0.2, random_state=42):
    ...
    for scaler_name, scaler in scalers.items():
        ...
        model.fit(X_train_scaled, y_train, epochs=300,
                  batch_size=32, validation_split=0.1, callbacks=[es])
...
if __name__ == "__main__":
    ...
    for scaler_name, data in scaled_results.items():
        ...
        print(f"Model ({scaler_name}) Root Mean Squared Error on test set: {mse:.2f}")

'''
--- Results for MinMaxScaler ---
X_train_scaled shape: (8708, 8)
X_test_scaled shape: (2178, 8)
Model (MinMaxScaler) Root Mean Squared Error on test set: 150.39

--- Results for StandardScaler ---
X_train_scaled shape: (8708, 8)
X_test_scaled shape: (2178, 8)
Model (StandardScaler) Root Mean Squared Error on test set: 145.96

--- Results for MaxAbsScaler ---
X_train_scaled shape: (8708, 8)
X_test_scaled shape: (2178, 8)
Model (MaxAbsScaler) Root Mean Squared Error on test set: 152.63

--- Results for RobustScaler ---
X_train_scaled shape: (8708, 8)
X_test_scaled shape: (2178, 8)
Model (RobustScaler) Root Mean Squared Error on test set: 148.04
'''