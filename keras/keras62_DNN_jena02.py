"""
    기존 데이터에서 y를 144개(1일치)를 맞춰야 하므로,
    
    y데이터를 위로 144개 shift하여 새로운 행렬 데이터 형태로 만들어서
    144개를 예측한다.
"""
...
csv = pd.read_csv(path + 'data_cleaned.csv', index_col=0)
test_csv = pd.read_csv(path + 'data_cleaned.csv', index_col=0)
...
# print(x.shape, y.shape)  # (41883, 1872) (41883, 144)
# print(test.shape)       # (1, 1872)
...
model = Sequential()
model.add(Dense(75, input_dim=13))
model.add(Dense(25))
...
model.fit(x_train, y_train, batch_size=100, epochs=1000,validation_split=0.2,
...
try:
    sub_csv.to_csv(path + 'jena_최준혁2_submit.csv', index=False)
    print("파일 저장 성공")
