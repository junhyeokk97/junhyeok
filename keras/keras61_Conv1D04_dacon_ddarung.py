...
x = x.to_numpy().reshape(x.shape[0], x.shape[1], 1)    # (1459, 9) >> (1459, 9, 1)
y = y.to_numpy().reshape(y.shape[0], 1)    # (1459, 1)
...
#2. 모델구성
model = Sequential()
model.add(Conv1D(10, kernel_size=2, input_shape=(9,1)))
model.add(Conv1D(9, 8))
model.add(Flatten())
model.add(Dense(100))
model.add(Dense(50))
model.add(Dense(20))
...
str = time.time()
model.fit(x,y, epochs=500, batch_size=32, validation_split=0.2, verbose=2)
end = time.time()
...
# loss :  2891.589599609375
# r2 스코어 :  0.5306145377190614
# 걸린시간 : 91.87632966041565

# loss:  3518.563720703125
# r2:  0.5548256574926167
# RMSE:  59.31748375981922
# 걸린시간:  65.74972248077393