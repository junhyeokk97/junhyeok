...
model = nn.Sequential(nn.Linear(9,10),
                      nn.ReLU(),
                      nn.Linear(10,10),
                      nn.ReLU(),
                      nn.Linear(10,10),
...
print('NaN in y_predict:', np.isnan(y_predict).sum())
print('NaN in y_test:', np.isnan(y_test).sum())

if np.isnan(y_predict).any() or np.isnan(y_test).any():
    print("Warning: NaN detected in predictions or true values!")
else:
    r2 = r2_score(y_test, y_predict)
    print('R2 score:', r2)