...
class LSTM(nn.Module):
    def __init__(self):
        ...
        self.rnn_layer1 = nn.LSTM(
            ...
            batch_first=False,  # feature timesteps batch -->> batch feature timesteps // default = False
    ...
    def forward(self, x, h0=None, c0=None):
        if h0 == None:      # 비교연산 ==
            h0 = torch.zeros(1, x.size(0), 32).to(DEVICE)
                    # ( num_layers, batch_size(데이터 갯수, hidden_size)
            c0 = torch.zeros(1, x.size(0), 32).to(DEVICE)
        x, (hn, cn) = self.lstm_layer1(x, (h0, c0))      # h0 = h는 hidden   >   RNN은 순환 신경망이므로 출력 값의 개수가 늘어난다.
        x = self.relu(x)
