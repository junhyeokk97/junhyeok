...
def preprocess_data(train_df, test_df, building_df):
    ...
    # 기본 시간/결측/휴일/용량/선형 HDD·CDD/천문·날씨 지표까지 개별 적용
    for df in (train_df, test_df):
        _add_time_features(df)
        _fill_weather_missing(df)
        _add_holiday_features(df)
        _add_capacity_building_features(df)
        _add_degree_days(df)
        _add_extra_weather_calendar_features_noaa(df)

    # ★ train+test concat 후 래그/롤링/구간 HDD·CDD/절대습도/상호작용 생성
    comb = pd.concat([train_df, test_df], axis=0, ignore_index=True)
    comb = _add_weather_lags_rollups(comb, lags=(1,3,6), rolls=(3,6,24))
    comb = _add_piecewise_degree_days(comb)
    comb = _add_abs_humidity(comb)
    comb = _add_interactions(comb)

    # 다시 split
    train_df = comb.iloc[:len(train_df)].copy()
    test_df  = comb.iloc[len(train_df):].copy()

    # 라벨 인코딩(건물유형)
    if '건물유형' in train_df.columns:
        le = LabelEncoder()
        combined = pd.concat([train_df['건물유형'], test_df['건물유형']])
        le.fit(combined.astype(str))
        train_df['건물유형'] = le.transform(train_df['건물유형'].astype(str))
        test_df['건물유형']  = le.transform(test_df['건물유형'].astype(str))

    # ➊ 타깃 누수 없는 타깃 평균 인코딩 (train에서만 계산 → train/test에 merge)
    if 'kwh_mean_bld_hour' not in train_df.columns:
        mean_bld_hour = _mean_on(train_df, ['건물번호', 'hour'], '전력소비량(kWh)')
        mean_bld_hour = mean_bld_hour.rename(columns={'mean': 'kwh_mean_bld_hour'})

        train_df = train_df.merge(mean_bld_hour, on=['건물번호', 'hour'], how='left')
        test_df  = test_df.merge(mean_bld_hour,  on=['건물번호', 'hour'], how='left')

        # 테스트에서 미존재 조합 보정: train 평균의 전체 평균으로 채움(누수 방지)
        global_mean = train_df['kwh_mean_bld_hour'].mean()
        test_df['kwh_mean_bld_hour'] = test_df['kwh_mean_bld_hour'].fillna(global_mean)

    # ➋ 외생(날씨) 피처의 그룹 평균 (누수 우려 없음 → train+test 합쳐서 계산)
    if 'T_mean_bld_hour' not in train_df.columns and '기온(°C)' in train_df.columns:
        comb2 = pd.concat(
            [train_df[['건물번호', 'hour', '기온(°C)']],
             test_df[['건물번호', 'hour', '기온(°C)']]],
            axis=0, ignore_index=True
        )
        temp_mean = _mean_on(comb2, ['건물번호', 'hour'], '기온(°C)').rename(columns={'mean': 'T_mean_bld_hour'})

        train_df = train_df.merge(temp_mean, on=['건물번호', 'hour'], how='left')
        test_df  = test_df.merge(temp_mean,  on=['건물번호', 'hour'], how='left')
    ...
    def load_params(self, filepath):
        try:
            ...
            if isinstance(save_data, dict):
                ...
                self.ensemble_weights = save_data.get('ensemble_weights', self.ensemble_weights)
                self.features = save_data.get('selected_features', self.features)
                self.feature_manager.importance_scores = save_data.get('feature_importances', {})
...
def main():
    ...
    try:
        ...
        logger.error(f"Optuna 전역 최적화 실패: {e}")
        raise
