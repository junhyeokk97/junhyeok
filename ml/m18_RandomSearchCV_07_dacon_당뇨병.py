...
pd.DataFrame(model.cv_results_).sort_values('rank_test_score',    # rank_test_score 기준으로 오름차순 정렬.
                                                  ascending=True).to_csv(path + 'm18_rs_07_dacon_당뇨병_cv_results.csv')

path = './_save/m15_cv_results/'
joblib.dump(model.best_estimator_, path + 'm18_07_best_model.joblib')  # 가중치 저장.

# 최적의 파라미터:  {'max_depth': 6, 'learning_rate': 0.1}
# best_score:  0.7351465201465202
