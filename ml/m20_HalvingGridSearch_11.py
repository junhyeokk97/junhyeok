...
# LabelEncoder를 사용하여 y의 클래스 레이블을 0부터 시작하는 연속적인 값으로 변환
encoder = LabelEncoder()
y = encoder.fit_transform(y)

# 데이터 분할
x_train, x_test, y_train, y_test = train_test_split(x, y, train_size=0.8, random_state=55, stratify=y)
...
model = HalvingGridSearchCV(xgb, parameters, cv = kfold, # 54 * 5 = 270번 
                     verbose = 1,
                     refit=True, # 1번
                     n_jobs = -1, # 271번 
                     random_state=33,
                     factor = 2, # 배수: min_resources * factor //
                                 # n_candidates / factor
                     min_resources = 30, # 1 iter때의 최소 훈련 행의 갯수
                     max_resources = 1437 # 데이터 행의 갯수(n_samples)
...
print(pd.DataFrame(model.cv_results_).sort_values(
    'rank_test_score',
    ascending = True).to_csv(path + 'm20_00_rs_cv_11.csv',index = False)) 


import joblib

joblib.dump(model.best_estimator_,path + 'm20_11_best_model.joblib' )


# best_score :  0.9456533595113438
# model.score :  0.9638888888888889
# accuracy_score :  0.9638888888888889
# best_acc_score : 0.9638888888888889
# 걸린 시간: 15.0 초