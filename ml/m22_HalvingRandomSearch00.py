...
model = HalvingRandomSearchCV(xgb, parameters, cv = kfold, # 54 * 5 = 270번 
                     ...
                     max_resources = 120 # 데이터 행의 갯수(n_samples)
...
print(pd.DataFrame(model.cv_results_).sort_values(
    'rank_test_score',
    ascending = True).to_csv(path + 'm22_00_rs_cv_00.csv',index = False)) 


import joblib

joblib.dump(model.best_estimator_,path + 'm22_00_best_model.joblib' )
