...
model = HalvingGridSearchCV(xgb, parameters, cv=kfold,   # 54 * 5 = 270
                    ...
                    #  n_iter=11,
                     ...
                     max_resources=150, # 데이터 행의 개수(n_sample)
                     )
...
pd.DataFrame(model.cv_results_).sort_values('rank_test_score',    # rank_test_score 기준으로 오름차순 정렬.
                                                  ascending=True).to_csv(path + 'm20_rs_03_diabetes_cv_results.csv')

path = './_save/m15_cv_results/'
joblib.dump(model.best_estimator_, path + 'm20_03_best_model.joblib')  # 가중치 저장.
