...
model = HalvingGridSearchCV(xgb, parameters, cv = kfold, # 54 * 5 = 270번 
                     ...
                     max_resources = 353 # 데이터 행의 갯수(n_samples)
...
print(pd.DataFrame(model.cv_results_).sort_values(
    'rank_test_score',
    ascending = True).to_csv(path + 'm20_00_rs_cv_03.csv',index = False)) 


import joblib

joblib.dump(model.best_estimator_,path + 'm20_03_best_model.joblib' )



# best_score :  0.3956281152404558
# model.score :  0.4484834572711609
# best_score :  0.3956281152404558
# model.score :  0.4484834572711609
