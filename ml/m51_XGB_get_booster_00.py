...
aaa = model.get_booster().get_score(importance_type='gain')     # weight로 설정 시 split 빈도 수 개념
...
aaa= list(aaa.values())
print(type(aaa))
bbb = [aaa.feature_importances_ for model in aaa]
ccc = np.mean(bbb, axis=0)

scaler = MinMaxScaler()
ddd = scaler.fit_transform(ccc.reshape(-1, 1)).flatten()

# ② 오름차순 정렬
thresholds = np.sort(ddd.feature_importances_) #오름차순
...
for i in thresholds:
    thresholds = np.sort(thresholds.feature_importances_)
