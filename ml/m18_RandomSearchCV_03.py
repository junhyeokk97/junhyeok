...
print(pd.DataFrame(model.cv_results_).sort_values(
    'rank_test_score',
    ascending = True).to_csv(path + 'm18_00_rs_cv_03.csv',index = False)) 


import joblib

joblib.dump(model.best_estimator_,path + 'm18_03_best_model.joblib' )
# model 이 아닌 model.best_estimator_ 로 저장한 이유는 model로 저장하면 gridsearch의 자잘한 파라미터까지 전부 포함 귀찮아짐 

# best_score :  0.37129910542961686
