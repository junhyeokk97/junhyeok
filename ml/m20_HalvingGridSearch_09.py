...
model = HalvingGridSearchCV(xgb, parameters, cv = kfold, # 54 * 5 = 270번 
                     ...
                     max_resources = 142 # 데이터 행의 갯수(n_samples)
...
print(pd.DataFrame(model.cv_results_).sort_values(
    'rank_test_score',
    ascending = True).to_csv(path + 'm20_00_rs_cv_09.csv',index = False)) 


import joblib

joblib.dump(model.best_estimator_,path + 'm20_09_best_model.joblib' )


# best_score :  0.9818181818181818
