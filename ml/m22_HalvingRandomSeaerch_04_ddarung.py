...
model = HalvingRandomSearchCV(xgb, parameters, cv=kfold,   # 54 * 5 = 270
                    ...
                    #  n_iter=11,
                      factor = 4,    # 데이터는 세 배로 늘리고, 후보군 3분의 1로.
                                    ...
                                    #       n_candidates / factor
                     min_resources=50,  # 1 iter 때의 최소 훈련 행의 수
                     max_resources=230, # 데이터 행의 개수(n_sample)
                     )
...
pd.DataFrame(model.cv_results_).sort_values('rank_test_score',    # rank_test_score 기준으로 오름차순 정렬.
                                                  ascending=True).to_csv(path + 'm22_hrs_04_ddarung_cv_results.csv')

path = './_save/m15_cv_results/'
joblib.dump(model.best_estimator_, path + 'm22_04_best_model.joblib')  # 가중치 저장.
...
# best_score:  0.6423240822745289
# model.score:  0.7394006948877767
# r2_score:  0.7394006948877767
# time :  8
...
# best_score:  0.5268747023774739
# model.score:  0.6193740438573219
# r2_score:  0.6193740438573219
# time :  5