...
# -*- coding: utf-8 -*-
"""
Review FULL One-File (AI Hub 속성기반 감정분석 → 학습/평가 → 리뷰 DB 주입 → 세분 자동답글(의도 태그/심각도) → 콘솔 리포트/샘플 추론)

핵심 기능
- 데이터 로드/정규화: CSV/TSV/JSON/JSONL 자동 감지, (text, aspect, label[-1/0/1], doc_id)로 통일
- 학습·평가: TF-IDF(char 2~4) + LogisticRegression (간단/견고), report.txt/predictions.csv/model.joblib 저장
- DB 임포트: 정규화 데이터 → reviews(SQLite) 저장(테스트용: -1→★1, 0→★3, 1→★5)
- 자동 답글: aspect 그룹 × 감성 5단계(-2..+2) + 보조 의도 태그(refund/exchange/late_delivery/…)
- 콘솔 리포트: 일/주/월 KPI + 최근 Top N 출력
- 샘플 추론: --predict "문장" [--aspect "배송"] -> 감성 예측(모델 있으면 사용, 없으면 휴리스틱) + 답글 생성

의존성(최소): pandas scikit-learn tqdm
선택: joblib (모델 저장/로드), python-dateutil

빠른 사용
  # 0) 설치
  # pip install pandas scikit-learn tqdm python-dateutil joblib

  # 1) DB 초기화
  python review_full_onefile.py --init-db --db ./review_bot.sqlite

  # 2) 데이터 미리보기
  python review_full_onefile.py --data ./data/AIHUB_ATTRSENT --preview

  # 3) 학습·평가 + 모델 저장
  python review_full_onefile.py --data ./data/AIHUB_ATTRSENT --train --eval --out ./_save/attrsent

  # 4) DB 임포트(테스트용 파이프라인)
  python review_full_onefile.py --data ./data/AIHUB_ATTRSENT --import-to-db --db ./review_bot.sqlite

  # 5) 세분 자동답글 큐 생성 + 미리보기
  python review_full_onefile.py --data ./data/AIHUB_ATTRSENT --enqueue --db ./review_bot.sqlite
  python review_full_onefile.py --reply-preview --db ./review_bot.sqlite

  # 6) 콘솔 리포트(전일)
  python review_full_onefile.py --report --period daily --db ./review_bot.sqlite

  # 7) 샘플 추론 + 답글
  python review_full_onefile.py --predict "배송이 너무 늦었어요 환불하고 싶습니다" --aspect "배송" --model ./_save/attrsent/model.joblib

주의: 실서비스(네이버/구글) 대량 수집은 각 플랫폼 약관을 준수해야 합니다. 본 파일은 공개 데이터셋으로 파이프라인 검증용입니다.
"""
...
def _auto_columns(df: pd.DataFrame) -> Tuple[str, Optional[str], Optional[str], str]:
    def pick(cands):
        for k in cands:
            for c in df.columns:
                if c.lower()==k:
                    return c
        return None
    text_col = pick(TEXT_KEYS) or df.columns[0]
    label_col = pick(LABEL_KEYS)
    aspect_col = pick(ASPECT_KEYS)
    id_col = pick(ID_KEYS) or df.columns[0]
    return text_col, label_col, aspect_col, id_col


def _normalize(df: pd.DataFrame) -> pd.DataFrame:
    tcol, lcol, acol, idcol = _auto_columns(df)
    out = pd.DataFrame()
    ...
    if lcol and lcol in df.columns:
        ...
        y = raw.apply(map_pol)
        if y.isna().mean() > 0.5:
            # 0/1/2 등의 정수 스킴일 수 있으니 중앙값을 0으로 맵핑 시도
            try:
                y2 = raw.astype(int)
                uniq = sorted(set(int(v) for v in y2))
...
def load_dataset(data_dir: str, file_glob: Optional[str]=None) -> pd.DataFrame:
    d = Path(data_dir)
    if not d.exists():
        raise FileNotFoundError(d)
    pats = file_glob or "**/*.*"
    files = [p for p in d.glob(pats) if p.suffix.lower() in {".csv",".tsv",".json",".jsonl",".jl"}]
    if not files:
        raise RuntimeError("No data files (csv/tsv/json/jsonl)")
    dfs = []
    for f in files:
        try:
            df = _read_file(f)
...
def compose_reply(aspect_group: str, severity: int, intents: set) -> str:
...
def enqueue_replies_from_df(df: pd.DataFrame, db_path: str):
    ...
    for r in df.itertuples(index=False):
        ...
        reply      = compose_reply(aspect_g, severity, intents)
        try:
            conn.execute(
...
def predict_and_reply(text: str, aspect_hint: Optional[str], model_path: Optional[str]):
    ...
    intents = detect_intents(text)
    reply = compose_reply(asp_g, sev, intents)
...
if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(description="FULL One-file Review Baseline")
    ap.add_argument("--data", type=str, default=None, help="AIHub 데이터 디렉토리")
    ...
    df = None
    if args.data:
        df = load_dataset(args.data, args.glob)
        if args.preview:
            print(df.head(5))
    ...
    if args.enqueue:
        if df is None:
            print("(--enqueue를 사용하려면 --data 로 데이터를 로드하세요)")
        else:
