...
TEXT_KEYS  = ["text", "sentence", "document", "review", "contents", "rawtext", "raw_text", "본문", "내용"]
LABEL_KEYS = ["polarity", "label", "sentiment", "target_polarity", "y", "generalpolarity", "감성", "감정"]
ASPECT_KEYS = ["attribute", "aspect", "category", "target", "aspect_term", "maincategory", "aspects", "세부속성", "속성"]
...
def _normalize(
    ...
    # aspect 컬럼이 없고, Aspects(리스트)가 있으면 요약해서 채우기
    if ("aspect" not in out.columns or out["aspect"].isna().all()) and ("Aspects" in df.columns):
        def join_aspects(x):
            try:
                names = {str(d.get("Aspect")) for d in (x or []) if isinstance(d, dict) and d.get("Aspect")}
                return "|".join(sorted(names)) if names else None
            except Exception:
                return None
        out["aspect"] = df["Aspects"].apply(join_aspects)
