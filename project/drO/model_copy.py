#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Review FULL One-File (FINAL FIXED, Gemma-freeform ready)
- 데이터 정규화 → 키워드 → 학습/평가 → DB → (젬마3 자유형) 답글 → 리포트/산출물
- 산출물: normalized.csv / keywords_unigram.csv / keywords_bigram.csv / replies.csv / report.md / results.csv
"""

from __future__ import annotations
from pathlib import Path
from dataclasses import dataclass
from typing import Optional, Tuple, Dict, List
from hashlib import md5

import os, re, time, logging, random, sqlite3, json, itertools
import pandas as pd
import numpy as np
from tqdm import tqdm

from sklearn.model_selection import train_test_split
from sklearn.feature_extraction.text import TfidfVectorizer
from sklearn.linear_model import LogisticRegression
from sklearn.metrics import classification_report, f1_score

try:
    import requests  # LLM(ollama)
except Exception:
    requests = None

# -------------------- 안전 저장 유틸 --------------------
def _safe_csv(df: pd.DataFrame, path: Path, retries: int = 3, delay: float = 0.5):
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp = path.with_suffix(path.suffix + f".tmp.{os.getpid()}")
    for _ in range(retries):
        try:
            df.to_csv(tmp, index=False, encoding="utf-8-sig")
            os.replace(tmp, path)  # atomic move
            print(f"[SAVE] {path}")
            return path
        except PermissionError:
            time.sleep(delay)
    ts = time.strftime("%Y%m%d_%H%M%S")
    alt = path.with_name(path.stem + f"_{ts}" + path.suffix)
    df.to_csv(alt, index=False, encoding="utf-8-sig")
    print(f"[SAVE] {alt} (fallback; original locked)")
    return alt

def _safe_text_write(path: Path, text: str, retries: int = 3, delay: float = 0.5):
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp = path.with_suffix(path.suffix + f".tmp.{os.getpid()}")
    for _ in range(retries):
        try:
            tmp.write_text(text, encoding="utf-8")
            os.replace(tmp, path)
            print(f"[SAVE] {path}")
            return path
        except PermissionError:
            time.sleep(delay)
    ts = time.strftime("%Y%m%d_%H%M%S")
    alt = path.with_name(path.stem + f"_{ts}" + path.suffix)
    Path(alt).write_text(text, encoding="utf-8")
    print(f"[SAVE] {alt} (fallback; original locked)")
    return alt

# -------------------- 로그 --------------------
logging.basicConfig(
    level=logging.INFO,
    format="[%(asctime)s] %(levelname)s: %(message)s",
    datefmt="%H:%M:%S",
)
log = logging.getLogger("review-bot").info
logd = logging.getLogger("review-bot").debug

# -------------------- 설정 --------------------
@dataclass
class CFG:
    DB_PATH: str = "./review_bot.sqlite"
    PLATFORM_NAME: str = "aihub_attrsent"
    RANDOM_STATE: int = 42
    STRICT_FREEFORM: bool = False
    # LLM
    USE_FREEFORM: bool = False
    LLM_BACKEND: str = "none"           # none|ollama
    LLM_MODEL: str = "gemma-3n-E4B"
    MAX_REPLY_LEN: int = 300
    OLLAMA_URL: str = "http://localhost:11434/api/generate"

TEXT_KEYS = ["text", "sentence", "document", "review", "contents"]
LABEL_KEYS = ["polarity", "label", "sentiment", "target_polarity", "y"]
ASPECT_KEYS = ["attribute", "aspect", "category", "target", "aspect_term"]
ID_KEYS    = ["id", "review_id", "doc_id", "uid"]

POLARITY_MAP = {
    "positive": 1, "pos": 1, "긍정": 1, "1": 1, 1: 1,
    "neutral": 0, "neu": 0, "중립": 0, "0": 0, 0: 0,
    "negative": -1, "neg": -1, "부정": -1, "-1": -1, -1: -1,
}

# -------------------- 파일 읽기/정규화 --------------------
def _read_file(path: Path, sheet_name: Optional[str]=None) -> pd.DataFrame:
    suf = path.suffix.lower()
    if suf == ".csv":
        try:
            return pd.read_csv(path, encoding="utf-8")
        except UnicodeDecodeError:
            return pd.read_csv(path, encoding="cp949")
    if suf == ".tsv":
        try:
            return pd.read_csv(path, encoding="utf-8", sep="\t")
        except UnicodeDecodeError:
            return pd.read_csv(path, encoding="cp949", sep="\t")
    if suf in [".jsonl", ".jl"]:
        with open(path, "r", encoding="utf-8") as f:
            return pd.read_json(f, lines=True)
    if suf == ".json":
        try:
            with open(path, "r", encoding="utf-8") as f:
                return pd.read_json(f)
        except ValueError:
            with open(path, "r", encoding="utf-8") as f:
                return pd.read_json(f, lines=True)
    if suf == ".xlsx":
        try:
            return pd.read_excel(path, sheet_name=sheet_name, engine="openpyxl")
        except Exception:
            return pd.read_excel(path, sheet_name=sheet_name)
    if suf == ".xls":
        try:
            return pd.read_excel(path, sheet_name=sheet_name, engine="xlrd")
        except Exception as e:
            raise RuntimeError(".xls는 xlrd==1.2.0 필요 (pip install xlrd==1.2.0)") from e
    raise ValueError(f"Unsupported format: {path}")

def _auto_columns(
    df: pd.DataFrame,
    override_text: Optional[str]=None,
    override_label: Optional[str]=None,
    override_aspect: Optional[str]=None,
    override_id: Optional[str]=None,
) -> Tuple[str, Optional[str], Optional[str], str]:
    def pick(cands: List[str]):
        for k in cands:
            for c in df.columns:
                if c.lower() == k:
                    return c
        return None
    text_col  = override_text  or pick(TEXT_KEYS)  or df.columns[0]
    label_col = override_label or pick(LABEL_KEYS)
    aspect_col= override_aspect or pick(ASPECT_KEYS)
    id_col    = override_id    or pick(ID_KEYS)    or df.columns[0]
    return text_col, label_col, aspect_col, id_col

def _normalize(
    df: pd.DataFrame,
    override_text: Optional[str]=None,
    override_label: Optional[str]=None,
    override_aspect: Optional[str]=None,
    override_id: Optional[str]=None,
) -> pd.DataFrame:
    tcol, lcol, acol, idcol = _auto_columns(df, override_text, override_label, override_aspect, override_id)

    out = pd.DataFrame()
    out["text"] = df[tcol].astype(str)
    out["aspect"] = df[acol].astype(str) if acol and acol in df.columns else None

    if lcol and lcol in df.columns:
        raw = df[lcol]
        def map_pol(x):
            if isinstance(x, str):
                x = x.strip().lower()
            return POLARITY_MAP.get(x, np.nan)
        y = raw.apply(map_pol)
        if y.isna().mean() > 0.5:
            try:
                y2 = raw.astype(int)
                uniq = sorted({int(v) for v in y2})
                lo, hi = min(uniq), max(uniq)
                mapping = {lo: -1, hi: 1}
                if len(uniq) >= 3:
                    mapping[uniq[len(uniq)//2]] = 0
                y = y2.map(mapping)
            except Exception:
                y = pd.Series([np.nan]*len(df))
        out["label"] = y
    else:
        out["label"] = np.nan

    if idcol and idcol in df.columns and idcol != tcol:
        out["doc_id"] = df[idcol].astype(str)
    else:
        srcs = df.get("__srcfile__", pd.Series(["unknown"]*len(df)))
        out["doc_id"] = [
            md5((str(s) + "|" + str(p) + "|" + str(i)).encode("utf-8")).hexdigest()[:16]
            for i, (s, p) in enumerate(zip(out["text"], srcs))
        ]

    out = out[~out["text"].astype(str).str.strip().eq("")]
    out = out.drop_duplicates(subset=["doc_id"])
    return out.reset_index(drop=True)

def load_dataset(data_dir: str, file_glob: Optional[str]=None, sheet_name: Optional[str]=None, col_map: Optional[Dict[str,str]]=None) -> pd.DataFrame:
    d = Path(data_dir)
    if not d.exists():
        raise FileNotFoundError(d)
    pats = file_glob or "**/*.*"
    files = [p for p in d.glob(pats) if p.suffix.lower() in {".csv",".tsv",".json",".jsonl",".jl",".xlsx",".xls"}]
    if not files:
        raise RuntimeError("No data files (csv/tsv/json/jsonl/xlsx/xls)")

    dfs = []
    for f in tqdm(files, desc="📥 Reading files", unit="file"):
        try:
            df = _read_file(f, sheet_name=sheet_name)
            df["__srcfile__"] = str(f)
            dfs.append(df)
        except Exception as e:
            print(f"skip: {f} ({e})")
    raw = pd.concat(dfs, axis=0, ignore_index=True)

    col_map = col_map or {}
    norm = _normalize(
        raw,
        override_text  = col_map.get("text"),
        override_label = col_map.get("label"),
        override_aspect= col_map.get("aspect"),
        override_id    = col_map.get("id"),
    )
    print(f"Loaded {len(raw)} → normalized {len(norm)} (files={len(files)})")
    return norm

def load_single_file(file_path: str, sheet_name: Optional[str]=None, col_map: Optional[Dict[str,str]]=None) -> pd.DataFrame:
    p = Path(file_path)
    if not p.exists():
        raise FileNotFoundError(p)
    df = _read_file(p, sheet_name=sheet_name)
    df["__srcfile__"] = str(p)
    col_map = col_map or {}
    norm = _normalize(
        df,
        override_text  = col_map.get("text"),
        override_label = col_map.get("label"),
        override_aspect= col_map.get("aspect"),
        override_id    = col_map.get("id"),
    )
    print(f"Loaded 1 file({p.name}) → normalized {len(norm)}")
    return norm

# ===== 문맥형 감성 분류 =====
LABEL3_STR = {-1: "부정", 0: "중립", 1: "긍정"}
LABEL5_STR = {-2: "매우 부정", -1: "부정", 0: "중립", 1: "긍정", 2: "매우 긍정"}

def _score_by_lexicon(text: str) -> float:
    t = (text or "").strip()
    if not t:
        return 0.0
    pos = {"좋","만족","추천","최고","감동","예쁘","가볍","편하","빠르","튼튼","깔끔"}
    neg = {"불만","환불","최악","파손","늦","지연","별로","불량","짜증","불편","누락","실망"}
    negators  = {"안","못","않","아니","없","미흡","부족"}
    intensify = {"매우","엄청","정말","완전","너무","굉장히","진짜"}
    weaken    = {"조금","살짝","약간","그럭저럭"}
    contrast  = {"하지만","근데","그러나","다만","반면","그런데"}

    clauses = re.split(r"(하지만|근데|그러나|다만|반면|그런데)", t)
    base_w, weights, parts, pending = 1.0, [], [], False
    for seg in clauses:
        if seg in contrast:
            pending = True
            continue
        if not seg.strip():
            continue
        parts.append(seg)
        if pending:
            base_w *= 1.4
            pending = False
        weights.append(base_w)

    def clause_score(s: str) -> float:
        tokens = re.findall(r"[가-힣A-Za-z0-9]+", s)
        sc = 0.0
        for i, w in enumerate(tokens):
            ctx_tokens = tokens[max(0, i-5):i]
            ctx = "".join(ctx_tokens)
            has_neg = any(n in ctx for n in negators)

            pv = 1.0 if any(p in w for p in pos) else 0.0
            nv = 1.0 if any(n in w for n in neg) else 0.0

            if any(xx in ctx for xx in intensify):
                pv *= 1.3; nv *= 1.3
            if any(xx in ctx for xx in weaken):
                pv *= 0.7; nv *= 0.7

            val = pv - nv
            if has_neg:
                val *= -1.0
            sc += val

        if "!" in s: sc *= 1.15
        if "??" in s or "?!" in s: sc *= 1.1
        return sc

    total = 0.0
    if parts:
        for w, seg in zip(weights, parts):
            total += clause_score(seg) * w
    else:
        total = clause_score(t)

    if total > 0.6: return 1.0
    if total < -0.6: return -1.0
    return 0.0

def _llm_sentiment(text: str) -> Optional[int]:
    if CFG.LLM_BACKEND != "ollama" or requests is None:
        return None
    prompt = (
        "다음 한국어 리뷰의 전반적 감성을 한 가지로 분류하세요. "
        "가능한 라벨: -1(부정), 0(중립), 1(긍정). 숫자만 출력.\n\n"
        f"리뷰: {text}\n\n정답:"
    )
    try:
        r = requests.post(
            CFG.OLLAMA_URL,
            json={"model": CFG.LLM_MODEL, "prompt": prompt, "stream": False},
            timeout=30,
        )
        if r.status_code != 200:
            return None
        s = (r.json().get("response") or r.json().get("text") or "").strip()
        tok = s.split()[0]
        if tok in {"-1", "0", "1"}:
            return int(tok)
    except Exception:
        return None
    return None

def infer_label_context(text: str, model_pack=None) -> int:
    if model_pack is not None:
        try:
            vec, mdl = model_pack["vectorizer"], model_pack["model"]
            X = vec.transform([text or ""])
            yhat = int(mdl.predict(X)[0])
            return 1 if yhat > 0 else (-1 if yhat < 0 else 0)
        except Exception:
            pass
    y = _llm_sentiment(text)
    if y in (-1, 0, 1):
        return y
    return int(_score_by_lexicon(text))

def label3_to_severity5(label3: int, text: str) -> int:
    base = int(label3)
    tl = (text or "").lower()
    very_neg = ["최악","다시는","환불","법적","고발","사기","불량","파손","누락","분노","빡치"]
    very_pos = ["최고","완벽","강추","최애","대만족","완전 좋","인생템","감동","레전드"]
    if any(k in tl for k in very_neg): base = min(base, -1) - 1
    if any(k in tl for k in very_pos): base = max(base,  1) + 1
    return max(min(base, 2), -2)

# -------------------- 키워드 --------------------
_KO_STOP = set([
    "그리고","그러나","하지만","또한","또","그","이","저","것","수","등","및","에서","으로","부터","까지","에게","께",
    "은","는","이","가","을","를","에","의","도","만","보다","처럼","하다","되다","있다","없다","같다","거","좀","너무",
])

def _simple_tokens(s: str):
    toks = re.findall(r"[가-힣A-Za-z0-9]{2,}", (s or "").lower())
    return [t for t in toks if t not in _KO_STOP]

from collections import Counter
def keyword_summary_dataset(df: pd.DataFrame, topk: int=50):
    texts = df["text"].astype(str).tolist()
    uni = Counter(); bi = Counter()
    for t in texts:
        toks = _simple_tokens(t)
        uni.update(toks)
        bi.update([" ".join(x) for x in zip(toks, toks[1:])])
    top_uni = pd.DataFrame(uni.most_common(topk), columns=["token","count"])
    top_bi  = pd.DataFrame(bi.most_common(topk),  columns=["token","count"])
    return top_uni, top_bi

# -------------------- LLM 생성기 --------------------
def _truncate(s: str, n: int):
    return s if len(s) <= n else s[:n-1] + "…"

def _llm_generate(prompt: str) -> str:
    if CFG.LLM_BACKEND != "ollama":
        raise RuntimeError("LLM_BACKEND is not 'ollama'")
    if requests is None:
        raise RuntimeError("requests not installed")
    try:
        resp = requests.post(
            CFG.OLLAMA_URL,
            json={"model": CFG.LLM_MODEL, "prompt": prompt, "stream": False},
            timeout=60,
        )
        if resp.status_code != 200:
            raise RuntimeError(f"LLM HTTP {resp.status_code}: {resp.text[:200]}")
        data = resp.json()
        text = (data.get("response") or data.get("text") or "").strip()
        if not text:
            raise RuntimeError("LLM returned empty response")
        return text
    except Exception as e:
        if CFG.STRICT_FREEFORM:
            raise
        logging.getLogger("review-bot").warning(f"[LLM-FALLBACK] {e}")
        return ""

SENT_LABELS = ["매우부정", "부정", "중립", "긍정", "매우긍정"]

def _llm_generate_json(prompt: str, max_retry: int = 2) -> Optional[dict]:
    for _ in range(max_retry + 1):
        txt = _llm_generate(prompt)
        if not txt:
            continue
        s = txt.strip()
        if s.startswith("```"):
            s = s.strip("` \n")
            if s.lower().startswith("json"):
                s = s[4:].lstrip()
        try:
            left = s.find("{"); right = s.rfind("}")
            if left >= 0 and right >= 0 and right > left:
                s = s[left:right+1]
            data = json.loads(s)
            return data
        except Exception:
            continue
    return None

# -------------------- 의도/답글 --------------------
ASPECT_MAP = {
    "배송":  ["배송","배달","도착","택배","포장","수령","지연"],
    "품질":  ["품질","재질","내구","원단","마감","상태","불량","파손"],
    "가격":  ["가격","비싸","저렴","가성비","할인","프로모션"],
    "사이즈":["사이즈","크기","핏","길이","넉넉","타이트"],
    "CS":   ["응대","고객센터","교환","환불","상담","불친절"],
}

def _aspect_group(a: Optional[str]) -> str:
    if not a or a == "None":
        return "기타"
    a_low = str(a).lower()
    for k, vocab in ASPECT_MAP.items():
        for v in vocab:
            if v.lower() in a_low:
                return k
    return "기타"

INTENT_PATTERNS = {
    "refund":        r"환불|refund",
    "exchange":      r"교환|교체|교환해",
    "late_delivery": r"지연|늦|배송.*늦|배송.*지연",
    "damaged":       r"파손|깨졌|찢어|스크래치|불량",
    "missing":       r"누락|없었|빠졌",
    "rude":          r"불친절|무례|태도",
    "question":      r"\?|문의|가능하나요|알고 싶|어떻게",
    "feature":       r"추가|기능|되면 좋|개선|요청",
}

def detect_intents(text: str) -> set:
    t = (text or "").lower()
    tags = set()
    for name, pat in INTENT_PATTERNS.items():
        if re.search(pat, t, flags=re.I):
            tags.add(name)
    return tags

REPLY_TEMPLATES = {
    ("배송", -2): "배송 문제로 큰 불편을 드려 진심으로 사과드립니다. 주문번호를 알려주시면 즉시 조사하고 필요한 조치를 진행하겠습니다.",
    ("배송", -1): "배송 지연으로 불편을 드려 죄송합니다. 더 정확한 배송 안내와 처리 속도를 개선하겠습니다.",
    ("배송",  0): "배송 관련 의견 감사합니다. 보다 안정적인 배송을 위해 계속 점검하겠습니다.",
    ("배송",  1): "배송이 만족스러우셨다니 기쁩니다. 다음에도 빠르고 안전하게 전달드리겠습니다!",
    ("배송",  2): "배송이 매우 만족스러우셨다니 감사합니다. 늘 같은 경험을 드리겠습니다!",
    ("품질", -2): "제품 문제로 큰 실망을 드렸습니다. 불량/파손 사진을 보내주시면 즉시 교환·환불을 도와드리겠습니다.",
    ("품질", -1): "품질 이슈를 주셔서 감사합니다. 해당 로트를 점검하고 재발 방지에 반영하겠습니다.",
    ("품질",  0): "품질 관련 의견 감사합니다. 더 나은 기준을 위해 꾸준히 개선하겠습니다.",
    ("품질",  1): "품질을 좋게 평가해주셔서 감사합니다. 기대에 부응하겠습니다!",
    ("품질",  2): "최고의 평가에 감사드립니다. 품질로 계속 만족 드리겠습니다!",
    ("가격", -2): "가격으로 크게 실망을 드려 죄송합니다. 프로모션/혜택을 더 알기 쉽게 안내드리겠습니다.",
    ("가격", -1): "가격에 대한 의견 감사드리며, 합리적 가격 정책을 위해 검토하겠습니다.",
    ("가격",  0): "가격 관련 의견 감사합니다. 더 나은 가치를 드리겠습니다.",
    ("가격",  1): "가격 만족 리뷰 감사합니다. 가성비 좋은 상품으로 보답하겠습니다!",
    ("가격",  2): "최고의 가성비 평가 감사합니다. 좋은 품질을 합리적인 가격으로 제공하겠습니다!",
    ("사이즈",-2): "사이즈로 큰 불편을 드려 죄송합니다. 교환 절차와 상세 사이즈 상담을 도와드리겠습니다.",
    ("사이즈",-1): "사이즈 의견 감사드립니다. 상세 가이드를 더 정확히 안내하겠습니다.",
    ("사이즈", 0): "사이즈 관련 의견 감사합니다. 표기 개선에 반영하겠습니다.",
    ("사이즈", 1): "사이즈가 잘 맞으셨다니 다행입니다!",
    ("사이즈", 2): "최상의 착용감 평가 감사드립니다. 같은 핏으로 또 찾아뵐게요!",
    ("CS",   -2): "응대 과정에서 큰 불편을 드려 진심으로 사과드립니다. 관련 내용을 즉시 시정하고 책임 있게 조치하겠습니다.",
    ("CS",   -1): "응대 관련 불편을 드려 죄송합니다. 교육·프로세스를 개선하겠습니다.",
    ("CS",    0): "응대 관련 의견 감사합니다. 더 신속하고 정확히 도와드리겠습니다.",
    ("CS",    1): "만족스러운 상담이었다니 감사드립니다.",
    ("CS",    2): "최고의 응대 평가에 감사드립니다. 항상 같은 경험을 드리겠습니다!",
    ("기타", -2): "큰 불편을 드려 죄송합니다. 상세 상황을 알려주시면 즉시 해결하겠습니다.",
    ("기타", -1): "불편을 드려 죄송합니다. 더 나은 경험을 위해 개선하겠습니다.",
    ("기타",  0): "소중한 의견 감사합니다. 개선에 반영하겠습니다.",
    ("기타",  1): "좋은 리뷰 감사합니다. 더 나은 경험을 제공하겠습니다.",
    ("기타",  2): "최고의 평가 감사합니다! 기대에 더 크게 보답하겠습니다.",
}

INTENT_OVERLAYS = [
    ("refund",        "환불을 요청해 주셔서 확인 즉시 진행하겠습니다. 주문번호와 결제 정보를 DM/메일로 공유 부탁드립니다."),
    ("exchange",      "교환 요청 건 확인했습니다. 절차와 반송 라벨을 안내드리겠습니다."),
    ("late_delivery", "배송 지연에 대해 사과드립니다. 현재 위치를 확인해 즉시 안내 드리겠습니다."),
    ("damaged",       "파손/불량으로 불편을 드려 죄송합니다. 사진을 보내주시면 교환/환불을 빠르게 처리하겠습니다."),
    ("missing",       "구성품 누락 건 확인 즉시 보완 발송하겠습니다. 부족한 항목을 알려주세요."),
    ("rude",          "응대 과정에서 불편을 드려 죄송합니다. 교육과 모니터링을 강화하겠습니다."),
    ("question",      "문의 주신 사항에 대해 자세히 안내드리겠습니다. 추가로 궁금하신 점도 편히 말씀해주세요."),
    ("feature",       "소중한 제안 감사합니다. 관련 부서와 공유하여 검토하겠습니다."),
]

def extract_keywords(text: str, topk: int = 6) -> list:
    toks = _simple_tokens(text)
    if not toks: return []
    uni = Counter(toks)
    bi  = Counter([f"{a} {b}" for a,b in zip(toks, toks[1:])])
    cand = []
    cand.extend([(w,c) for w,c in uni.items()])
    cand.extend([(w,c*1.5) for w,c in bi.items()])
    cand = sorted(cand, key=lambda x: x[1], reverse=True)
    out = []
    for w,_ in cand:
        if all(w not in o and o not in w for o in out):
            out.append(w)
        if len(out) >= topk: break
    return out

def _infer_label_simple(text: str) -> int:
    t = (text or "").lower()
    pos = ["좋","만족","추천","최고","감동","great","love"]
    neg = ["불만","환불","최악","파손","늦","지연","별로","bad"]
    score = sum(k in t for k in pos) - sum(k in t for k in neg)
    return 1 if score>0 else (-1 if score<0 else 0)

def compose_reply(aspect_group: str, severity: int, intents: set, text_for_freeform: str = "") -> str:
    # ✅ 젬마 자유형 강제: 실패 시 예외 (템플릿 금지)
    if CFG.USE_FREEFORM:
        txt = compose_reply_freeform(aspect_group, severity, intents, text_for_freeform)
        if txt is None:
            # STRICT_FREEFORM=True일 때 _llm_generate가 이미 raise 했을 수도 있지만,
            # 혹시 모를 None도 여기서 확실히 막아준다.
            raise RuntimeError("LLM freeform reply failed (no fallback by design).")
        return txt

    # ⛔️ freeform이 꺼져 있을 때만 템플릿 사용
    base = REPLY_TEMPLATES.get((aspect_group, severity)) \
        or REPLY_TEMPLATES.get((aspect_group, 0)) \
        or REPLY_TEMPLATES.get(("기타", 0))
    overlays = []
    for key, text in INTENT_OVERLAYS:
        if key in intents:
            overlays.append(text)
        if len(overlays) >= 2:
            break
    if overlays:
        return base + "\n" + "\n".join(overlays)
    return base

def compose_reply_freeform(aspect_group: str, severity: int, intents: set, text: str) -> Optional[str]:
    kws = extract_keywords(text, topk=6)
    prompt = (
        "역할: 전자상거래 고객응대 담당자. 한국어, 2~4문장. 사람처럼 자연스럽게.\n"
        f"- 리뷰 핵심 키워드: {kws}\n"
        f"- 속성 그룹: {aspect_group}\n"
        f"- 감성/강도: {severity} (-2 매우 부정 ~ +2 매우 긍정)\n"
        f"- 의도 태그: {sorted(list(intents))}\n"
        "스타일 지침(매우 중요):\n"
        "1) 상투어/형식어 금지: '소중한 의견 감사합니다', '더 나은 경험', '문의 주신 사항', '감사드립니다', '불편을 드려 죄송' 같은 문구 쓰지 말 것.\n"
        "2) 이모지/과장/기업 홍보 문구 금지.\n"
        "3) 복붙 느낌 금지. 리뷰의 맥락을 한두 단어로 정확히 언급하고, 필요한 안내만 간결히.\n"
        "4) 필요할 때만 사과나 감사. 무조건 넣지 말 것.\n"
        "5) 반말/지나친 친근체 금지. 존댓말로 담백하게.\n"
        f"- 길이: {CFG.MAX_REPLY_LEN}자 이내.\n"
        "출력: 답글 본문만. 머리말/리스트/따옴표 금지."
    )
    txt = _llm_generate(prompt)  # 실패 시 "" 또는 예외
    if not txt and CFG.STRICT_FREEFORM:
        raise RuntimeError("LLM did not return text (strict freeform)")
    if txt:
        return _truncate(txt, CFG.MAX_REPLY_LEN)
    return None

# -------- Gemma가 키워드/감성/답글을 한 번에 판단 --------
def analyze_with_llm(text: str, aspect_group: str) -> dict:
    prompt = (
        "너는 한국어 CS 담당자야. 아래 고객 리뷰를 읽고 JSON으로만 답해.\n"
        "요구사항:\n"
        f"- aspect_group: {aspect_group}\n"
        "- keywords: 핵심 키워드 3~6개 (문자열 배열)\n"
        "- sentiment: 하나만 선택 ['매우부정','부정','중립','긍정','매우긍정']\n"
        "- reply: 리뷰 맥락에 딱 맞춘 답글(2~4문장, 이모지X, 과장X, 회사 내부 표현X)\n"
        "출력은 반드시 다음 JSON 스키마만 사용:\n"
        "{\n"
        '  "keywords": ["..."],\n'
        '  "sentiment": "중립",\n'
        '  "reply": "..." \n'
        "}\n"
        "-----\n"
        f"리뷰:\n{text}\n"
        "-----\n"
        "JSON만 출력해."
    )
    data = _llm_generate_json(prompt)
    if not data:
        kws = extract_keywords(text, topk=6)
        base_label = _infer_label_simple(text)
        sev = estimate_severity(base_label, text)
        sent = { -2:"매우부정",-1:"부정",0:"중립",1:"긍정",2:"매우긍정"}[sev]
        reply = compose_reply_freeform(aspect_group, sev, detect_intents(text), text) or \
                compose_reply(aspect_group, sev, detect_intents(text), text)
        return {"keywords": kws, "sentiment": sent, "reply": reply}
    kws = data.get("keywords") or []
    if not isinstance(kws, list):
        kws = [str(kws)]
    kws = [str(x) for x in kws][:6]
    sent = str(data.get("sentiment") or "중립")
    if sent not in SENT_LABELS:
        sent = "중립"
    rep = str(data.get("reply") or "").strip()
    if not rep:
        # 비면 최소 폴백
        sev = 0
        rep = compose_reply(aspect_group, sev, detect_intents(text), text)
    return {"keywords": kws, "sentiment": sent, "reply": rep}

# -------------------- 학습/평가 --------------------
def train_eval(df: pd.DataFrame, out_dir: Optional[str]=None):
    data = df.dropna(subset=["label"]).reset_index(drop=True)
    if len(data) < 100:
        print("(Not enough labeled samples to train)")
        return None

    X = data["text"].astype(str).tolist()
    y = data["label"].astype(int).tolist()

    X_tr, X_te, y_tr, y_te = train_test_split(
        X, y, test_size=0.2, random_state=CFG.RANDOM_STATE, stratify=y
    )

    t0 = time.time()
    log("Vectorizing (TF-IDF char 2-4)…")
    vec = TfidfVectorizer(analyzer="char", ngram_range=(2, 4), min_df=2, max_df=0.95)
    Xtr = vec.fit_transform(X_tr)
    Xte = vec.transform(X_te)
    log(f"Vectorized in {time.time()-t0:.2f}s, dim={Xtr.shape[1]:,}")

    log("Training LogisticRegression …")
    clf = LogisticRegression(max_iter=200)
    clf.fit(Xtr, y_tr)
    pred = clf.predict(Xte)

    f1_macro = f1_score(y_te, pred, average="macro")
    report_str = classification_report(y_te, pred, digits=4)

    print("\n=== Classification Report (-1,0,1) ===")
    print(report_str)
    print(f"F1-macro: {f1_macro:.4f}")

    model_pack = {"vectorizer": vec, "model": clf}

    if out_dir:
        Path(out_dir).mkdir(parents=True, exist_ok=True)
        pd.DataFrame({"text": X_te, "y_true": y_te, "y_pred": pred}).to_csv(
            Path(out_dir) / "predictions.csv", index=False, encoding="utf-8-sig"
        )
        with open(Path(out_dir) / "report.txt", "w", encoding="utf-8") as f:
            f.write(report_str + f"\nF1-macro: {f1_macro:.6f}\n")
        try:
            import joblib
            joblib.dump(model_pack, Path(out_dir) / "model.joblib")
            print(f"Saved: {out_dir}/model.joblib")
        except Exception:
            print("(joblib not installed; skip model save)")

    return model_pack

# -------------------- DB --------------------
DDL = """
CREATE TABLE IF NOT EXISTS reviews (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  platform TEXT NOT NULL,
  review_id TEXT NOT NULL,
  author TEXT,
  rating REAL,
  content TEXT,
  created_at TEXT,
  crawled_at TEXT NOT NULL,
  UNIQUE(platform, review_id)
);
CREATE TABLE IF NOT EXISTS replies (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  platform TEXT NOT NULL,
  review_id TEXT NOT NULL,
  reply_text TEXT NOT NULL,
  replied_at TEXT,
  status TEXT NOT NULL DEFAULT 'pending',
  sentiment TEXT,
  keywords TEXT
);
CREATE TABLE IF NOT EXISTS faq_answers (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  intent TEXT NOT NULL,
  topic TEXT NOT NULL,
  answer TEXT NOT NULL,
  updated_at TEXT NOT NULL
);
"""

def init_db(db_path: str):
    conn = sqlite3.connect(db_path)
    conn.executescript(DDL)
    conn.commit()
    conn.close()
    print(f"DB initialized: {db_path}")

def ensure_schema(db_path: str):
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
    cur.execute("PRAGMA table_info(replies)")
    cols = {r[1] for r in cur.fetchall()}
    alters = []
    if "sentiment" not in cols:
        alters.append("ALTER TABLE replies ADD COLUMN sentiment TEXT")
    if "keywords" not in cols:
        alters.append("ALTER TABLE replies ADD COLUMN keywords TEXT")
    for sql in alters:
        try:
            cur.execute(sql)
        except Exception:
            pass
    conn.commit()
    conn.close()

def import_to_db(df: pd.DataFrame, db_path: str):
    conn = sqlite3.connect(db_path)
    conn.execute("PRAGMA journal_mode=WAL;")
    def to_star(x):
        if pd.isna(x): return None
        return {-1:1, 0:3, 1:5}.get(int(x), None)
    now = pd.Timestamp.utcnow().isoformat()
    n_new = 0
    for r in tqdm(df.itertuples(index=False), total=len(df), desc="🗄️ Importing to DB"):
        try:
            conn.execute(
                "INSERT OR IGNORE INTO reviews(platform, review_id, author, rating, content, created_at, crawled_at) VALUES(?,?,?,?,?,?,?)",
                (CFG.PLATFORM_NAME, r.doc_id, None, to_star(getattr(r, "label", None)), r.text, None, now)
            )
            n_new += conn.total_changes > 0
        except Exception:
            pass
    conn.commit()
    conn.close()
    print(f"Imported to DB: {n_new} rows → {db_path}")

# -------------------- 큐잉/프리뷰 --------------------
def enqueue_replies_from_df(df: pd.DataFrame, db_path: str, model_pack=None, limit: Optional[int]=None):
    ensure_schema(db_path)
    conn = sqlite3.connect(db_path)
    cnt = 0
    it = df.itertuples(index=False)
    total = len(df) if limit is None else min(int(limit), len(df))
    if limit:
        it = itertools.islice(it, int(limit))

    for r in tqdm(it, total=total, desc="✉️ Enqueue replies"):
        doc_id = r.doc_id
        text   = getattr(r, "text", "") or ""
        base_label3 = infer_label_context(text, model_pack=model_pack)
        sev         = label3_to_severity5(base_label3, text)
        aspect_g    = _aspect_group(getattr(r, "aspect", None))
        if CFG.USE_FREEFORM and CFG.LLM_BACKEND == "ollama":
            analyzed = analyze_with_llm(text, aspect_g)
            reply = analyzed["reply"] or compose_reply(aspect_g, sev, detect_intents(text), text)
            keywords_str = ", ".join(analyzed["keywords"])
            sentiment_str = analyzed["sentiment"]
        else:
            reply = compose_reply(aspect_g, sev, detect_intents(text), text_for_freeform=text)
            keywords_str = ", ".join(extract_keywords(text, topk=6))
            sentiment_str = { -2:"매우부정",-1:"부정",0:"중립",1:"긍정",2:"매우긍정"}[sev]
        try:
            conn.execute(
                "INSERT OR IGNORE INTO replies(platform, review_id, reply_text, status, sentiment, keywords) VALUES(?,?,?,?,?,?)",
                (CFG.PLATFORM_NAME, doc_id, reply, "pending", sentiment_str, keywords_str)
            )
            cnt += conn.total_changes > 0
        except Exception:
            pass
    conn.commit()
    conn.close()
    print(f"Enqueued replies: {cnt}")

def reply_preview(db_path: str, limit: int=10):
    conn = sqlite3.connect(db_path)
    rows = conn.execute(
        "SELECT platform, review_id, reply_text FROM replies WHERE status='pending' ORDER BY id DESC LIMIT ?",
        (limit,)
    ).fetchall()
    conn.close()
    if not rows:
        print("(No pending replies)")
        return
    print(f"\n===== Reply Preview (latest {len(rows)}) =====")
    for i, r in enumerate(rows, 1):
        print(f"[{i}] ({r[0]}) review_id={r[1]}\n→ {r[2]}\n")

# -------------------- 리포트/산출 --------------------
from datetime import datetime, timedelta
try:
    from dateutil import tz
    SEOUL_TZ = tz.gettz("Asia/Seoul")
except Exception:
    SEOUL_TZ = None

def _period(period: str) -> Tuple[str,str]:
    now = datetime.now(SEOUL_TZ) if SEOUL_TZ else datetime.now()
    if period=="daily":
        s = (now - timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0)
        e = (now - timedelta(days=1)).replace(hour=23, minute=59, second=59, microsecond=0)
    elif period=="weekly":
        wd = now.weekday()
        e = (now - timedelta(days=wd+1)).replace(hour=23, minute=59, second=59, microsecond=0)
        s = e - timedelta(days=6)
    elif period=="monthly":
        first_this = now.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
        e = first_this - timedelta(seconds=1)
        s = (first_this - timedelta(days=1)).replace(day=1, hour=0, minute=0, second=0, microsecond=0)
    else:
        raise ValueError("period must be daily/weekly/monthly")
    return s.isoformat(), e.isoformat()

def report_stdout(db_path: str, period: str="daily", topn: int=10):
    s, e = _period(period)
    conn = sqlite3.connect(db_path)
    df = pd.read_sql_query(
        "SELECT platform, author, rating, content, created_at, crawled_at FROM reviews WHERE crawled_at BETWEEN ? AND ? ORDER BY crawled_at DESC",
        conn, params=(s, e)
    )
    conn.close()
    total = len(df)
    avg_rating = round(float(df["rating"].dropna().mean()), 3) if total else 0.0
    print("\n==============================")
    print(f"📊 Review Summary ({period})")
    print(f"기간: {s} ~ {e}")
    print("------------------------------")
    print(f"총 리뷰 수   : {total}")
    print(f"평균 평점    : {avg_rating if total else '-'}")
    print("------------------------------")
    if total:
        print(f"📝 최근 리뷰 Top {min(topn, total)}")
        for i, row in enumerate(df.head(topn).itertuples(index=False), 1):
            content = (row.content or "").strip().replace("\n"," ")
            if len(content) > 120:
                content = content[:120] + "…"
            print(f"[{i}] ({row.platform}) ★{row.rating or '-'} | {row.created_at or row.crawled_at}")
            print(f"     {content}")
    else:
        print("해당 기간 리뷰가 없습니다.")

def export_normalized(df: pd.DataFrame, out_dir: str):
    out = Path(out_dir) / "normalized.csv"
    _safe_csv(df, out)

def export_keywords(df: pd.DataFrame, out_dir: str, topk: int = 50):
    p = Path(out_dir)
    texts = df["text"].astype(str).tolist()
    uni = Counter(); bi = Counter()
    for t in texts:
        toks = _simple_tokens(t)
        uni.update(toks)
        bi.update([" ".join(x) for x in zip(toks, toks[1:])])
    u = pd.DataFrame(uni.most_common(topk), columns=["token","count"])
    b = pd.DataFrame(bi.most_common(topk),  columns=["token","count"])
    _safe_csv(u, p / "keywords_unigram.csv")
    _safe_csv(b, p / "keywords_bigram.csv")

def export_replies(db_path: str, out_dir: str, limit: Optional[int]=None):
    p = Path(out_dir); p.mkdir(parents=True, exist_ok=True)
    conn = sqlite3.connect(db_path)
    q = ("SELECT r.platform, r.review_id, v.content as review_text, "
         "r.sentiment, r.keywords, r.reply_text as reply, r.status, r.id "
         "FROM replies r "
         "LEFT JOIN reviews v ON v.review_id = r.review_id "
         "ORDER BY r.id DESC")
    if limit:
        q += f" LIMIT {int(limit)}"
    df = pd.read_sql_query(q, conn); conn.close()
    _safe_csv(df, p / "replies.csv")

def export_report_markdown(db_path: str, period: str, out_dir: str, topn: int=10):
    s, e = _period(period)
    conn = sqlite3.connect(db_path)
    df = pd.read_sql_query(
        "SELECT platform, author, rating, content, created_at, crawled_at FROM reviews WHERE crawled_at BETWEEN ? AND ? ORDER BY crawled_at DESC",
        conn, params=(s, e)
    )
    conn.close()
    total = len(df)
    avg_rating = round(float(df["rating"].dropna().mean()), 3) if total else 0.0
    lines = []
    lines.append(f"# Review Summary ({period})")
    lines.append(f"기간: {s} ~ {e}")
    lines.append("")
    lines.append(f"- 총 리뷰 수: **{total}**")
    lines.append(f"- 평균 평점: **{avg_rating if total else '-'}**")
    lines.append("")
    if total:
        lines.append(f"## 최근 리뷰 Top {min(topn, total)}")
        for i, row in enumerate(df.head(topn).itertuples(index=False), 1):
            content = (row.content or "").strip().replace("\n"," ")
            if len(content) > 140:
                content = content[:140] + "…"
            lines.append(f"{i}. ({row.platform}) ★{row.rating or '-'} | {row.created_at or row.crawled_at}")
            lines.append(f"   - {content}")
    out = Path(out_dir) / "report.md"
    _safe_text_write(out, "\n".join(lines))

def export_results(df: pd.DataFrame, out_dir: str, model_pack=None):
    p = Path(out_dir); p.mkdir(parents=True, exist_ok=True)
    rows = []
    for r in tqdm(df.itertuples(index=False), total=len(df), desc="📝 Build results"):
        text   = getattr(r, "text", "") or ""
        aspect = getattr(r, "aspect", None)

        label3 = infer_label_context(text, model_pack=model_pack)
        severity = label3_to_severity5(label3, text)

        aspect_g = _aspect_group(aspect)
        intents  = detect_intents(text)
        kws      = extract_keywords(text, topk=6)
        reply    = compose_reply(aspect_g, severity, intents, text_for_freeform=text)

        rows.append({
            "doc_id":        getattr(r, "doc_id"),
            "text":          text,
            "aspect":        aspect,
            "aspect_group":  aspect_g,
            "keywords":      ", ".join(kws),
            "label_num3":    label3,
            "label_str3":    LABEL3_STR.get(label3, "중립"),
            "severity_num5": severity,
            "severity_str5": LABEL5_STR.get(severity, "중립"),
            "intents":       ", ".join(sorted(list(intents))) if intents else "",
            "reply":         reply,
        })
    out = p / "results.csv"
    _safe_csv(pd.DataFrame(rows), out)

# -------------------- 샘플 추론 --------------------
def _load_model(model_path: Optional[str]):
    if not model_path: return None
    p = Path(model_path)
    if not p.exists():
        print(f"(model not found: {model_path})"); return None
    try:
        import joblib
        return joblib.load(p)
    except Exception as e:
        print(f"(failed to load model: {e})"); return None

def predict_and_reply(text: str, aspect_hint: Optional[str], model_path: Optional[str]):
    pack = _load_model(model_path)
    base_label3 = infer_label_context(text, model_pack=pack)
    severity    = label3_to_severity5(base_label3, text)
    asp_g       = _aspect_group(aspect_hint or "기타")
    intents     = detect_intents(text)
    reply       = compose_reply(asp_g, severity, intents, text_for_freeform=text)

    print("문장:", text)
    print("라벨(3):", base_label3, "/", LABEL3_STR.get(base_label3))
    print("Severity(5):", severity, "/", LABEL5_STR.get(severity))
    print("의도태그:", ", ".join(sorted(intents)) or "(없음)")
    print("--- 자동답글 ---\n" + reply)

# -------------------- 메인 --------------------
if __name__ == "__main__":
    import argparse, sys, datetime
    ap = argparse.ArgumentParser(description="FULL One-file Review Baseline (FINAL)")

    ap.add_argument("--verbose", action="store_true", help="자세한 진행 로그")
    ap.add_argument("--quiet", action="store_true", help="진행 로그 최소화")
    ap.add_argument("--strict-freeform", action="store_true",
                    help="LLM 실패 시 템플릿 폴백 없이 에러로 중단")
    ap.add_argument("--llm-url", type=str, default=None, help="Ollama generate URL")

    ap.add_argument("--freeform", action="store_true", help="자연어형 답글 생성(템플릿 대신)")
    ap.add_argument("--llm", choices=["none","ollama"], default="none", help="LLM 백엔드")
    ap.add_argument("--llm-model", type=str, default="gemma-3n-E4B", help="Ollama 모델명")
    ap.add_argument("--max-reply-len", type=int, default=300, help="답글 최대 길이")

    ap.add_argument("--data", type=str, default=None, help="디렉토리(재귀) 경로")
    ap.add_argument("--file", type=str, default=None, help="단일 파일 경로 (csv/tsv/json/jsonl/xlsx/xls)")
    ap.add_argument("--glob", type=str, default=None, help="파일 패턴 (예: **/train*.jsonl)")
    ap.add_argument("--sheet", type=str, default=None, help="엑셀 시트명")

    ap.add_argument("--text-col", type=str, default=None)
    ap.add_argument("--label-col", type=str, default=None)
    ap.add_argument("--aspect-col", type=str, default=None)
    ap.add_argument("--id-col", type=str, default=None)

    ap.add_argument("--init-db", action="store_true")
    ap.add_argument("--db", type=str, default=CFG.DB_PATH)

    ap.add_argument("--preview", action="store_true")
    ap.add_argument("--train", action="store_true")
    ap.add_argument("--eval", action="store_true")
    ap.add_argument("--out", type=str, default=None)

    ap.add_argument("--import-to-db", action="store_true")

    ap.add_argument("--enqueue", action="store_true")
    ap.add_argument("--enqueue-limit", type=int, default=None)

    ap.add_argument("--reply-preview", action="store_true")

    ap.add_argument("--report", action="store_true")
    ap.add_argument("--period", choices=["daily","weekly","monthly"], default="daily")

    ap.add_argument("--export-dir", type=str, default=None, help="산출물 저장 폴더")
    ap.add_argument("--export-normalized", action="store_true", help="정규화 데이터 CSV 저장")
    ap.add_argument("--export-keywords", action="store_true", help="키워드 상위 리스트 CSV 저장")
    ap.add_argument("--export-replies", action="store_true", help="DB의 답글 CSV 저장")
    ap.add_argument("--export-report", action="store_true", help="요약 리포트 Markdown 저장")

    ap.add_argument("--predict", type=str, default=None, help="한 문장 평가/답글 생성")
    ap.add_argument("--aspect", type=str, default=None, help="문장 관련 속성 힌트(예: 배송/품질/가격/사이즈/CS)")
    ap.add_argument("--model", type=str, default=None, help="model.joblib 경로(선택)")

    args = ap.parse_args()

    # 사전 반영
    CFG.STRICT_FREEFORM = bool(args.strict_freeform)
    if args.llm_url:
        CFG.OLLAMA_URL = args.llm_url

    # -------------------- 자동 실행 모드 --------------------
    def _is_auto_mode(a: argparse.Namespace) -> bool:
        if len(sys.argv) == 1:
            return True
        meaningful = any([
            a.data, a.file, a.init_db, a.import_to_db, a.enqueue,
            a.export_dir, a.report, a.predict, a.train, a.eval
        ])
        return not meaningful

    AUTO_DEFAULTS = {
        "data": r"C:\study25\project\Sample\02.라벨링데이터\SNS",
        "text_col": "RawText",
        "label_col": "GeneralPolarity",
        "aspect_col": "MainCategory",
        "id_col": "id",
        "db": r"C:\study25\project\review_bot.sqlite",
        "export_root": r"C:\study25\project\out",
        "use_freeform": True,
        "llm": "ollama",
        "llm_model": "gemma-3n-E4B",
        "period": "weekly",
    }

    if _is_auto_mode(args):
        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        export_dir = str(Path(AUTO_DEFAULTS["export_root"]) / f"auto_{ts}")
        Path(export_dir).mkdir(parents=True, exist_ok=True)

        logging.getLogger().setLevel(logging.INFO)
        log("🚀 Auto mode start (no CLI args detected)")

        CFG.USE_FREEFORM = AUTO_DEFAULTS["use_freeform"]
        CFG.LLM_BACKEND  = AUTO_DEFAULTS["llm"]
        CFG.LLM_MODEL    = AUTO_DEFAULTS["llm_model"]
        CFG.MAX_REPLY_LEN= int(args.max_reply_len or 300)

        db_path = AUTO_DEFAULTS["db"]
        col_map = {
            "text":   AUTO_DEFAULTS["text_col"],
            "label":  AUTO_DEFAULTS["label_col"],
            "aspect": AUTO_DEFAULTS["aspect_col"],
            "id":     AUTO_DEFAULTS["id_col"],
        }

        df = load_dataset(AUTO_DEFAULTS["data"], file_glob=None, sheet_name=None, col_map=col_map)
        print(df.head(5))

        init_db(db_path)
        import_to_db(df, db_path)
        enqueue_replies_from_df(df, db_path, model_pack=None)

        export_normalized(df, export_dir)
        export_keywords(df, export_dir, topk=50)
        export_results(df, export_dir, model_pack=None)
        export_replies(db_path, export_dir, limit=None)
        export_report_markdown(db_path, AUTO_DEFAULTS["period"], export_dir, topn=10)

        report_stdout(db_path, AUTO_DEFAULTS["period"])

        print("\n✅ Auto mode finished.")
        print(f"📂 Saved to: {export_dir}")
        print(f"🗄️ DB: {db_path}")
        sys.exit(0)

    # -------------------- 수동 모드 --------------------
    if args.verbose:
        logging.getLogger().setLevel(logging.DEBUG)
    elif args.quiet:
        logging.getLogger().setLevel(logging.WARNING)
    else:
        logging.getLogger().setLevel(logging.INFO)
    log("🚀 Start")

    CFG.USE_FREEFORM = bool(args.freeform)
    CFG.LLM_BACKEND  = args.llm
    CFG.LLM_MODEL    = args.llm_model
    CFG.MAX_REPLY_LEN= int(args.max_reply_len)

    col_map = {
        k: v for k, v in {
            "text": args.text_col,
            "label": args.label_col,
            "aspect": args.aspect_col,
            "id": args.id_col,
        }.items() if v
    }

    if args.init_db:
        init_db(args.db)

    df = None
    if args.file:
        df = load_single_file(args.file, sheet_name=args.sheet, col_map=col_map)
        if args.preview: print(df.head(5))
    elif args.data:
        df = load_dataset(args.data, file_glob=args.glob, sheet_name=args.sheet, col_map=col_map)
        if args.preview: print(df.head(5))

    model_pack = None
    if (args.train or args.eval) and df is not None:
        model_pack = train_eval(df, args.out)

    if args.import_to_db and df is not None:
        import_to_db(df, args.db)

    if args.enqueue:
        if df is None:
            print("(--enqueue 사용 시 --file 또는 --data 로 데이터를 먼저 로드하세요)")
        else:
            enqueue_replies_from_df(df, args.db, model_pack=model_pack, limit=args.enqueue_limit)

    if args.reply_preview:
        reply_preview(args.db)

    if args.export_dir:
        Path(args.export_dir).mkdir(parents=True, exist_ok=True)
        if args.export_normalized and df is not None:
            export_normalized(df, args.export_dir)
        if args.export_keywords and df is not None:
            export_keywords(df, args.export_dir, topk=50)
        if df is not None:
            export_results(df, args.export_dir, model_pack=model_pack)
        if args.export_replies:
            export_replies(args.db, args.export_dir, limit=None)
        if args.export_report:
            export_report_markdown(args.db, args.period, args.export_dir, topn=10)

    if args.report:
        report_stdout(args.db, args.period)

    if args.predict:
        predict_and_reply(args.predict, args.aspect, args.model)